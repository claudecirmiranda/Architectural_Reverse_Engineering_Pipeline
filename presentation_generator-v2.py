...
class PresentationGenerator:
    ...
    def collect_data(self):
        ...
        # Extrair estatísticas de gaps se disponível
        if self.data['stats']['has_gaps']:
            self.data['gap_stats'] = self._extract_gap_stats()
            print(f"   📊 Gaps: {self.data['gap_stats']['critical']} críticos, {self.data['gap_stats']['high']} altos")
        else:
            self.data['gap_stats'] = {'critical': 0, 'high': 0, 'medium': 0, 'low': 0}
    ...
    def _extract_gap_stats(self) -> Dict:
        """Extrai estatísticas do documento de Gap Analysis."""
        if not self.data.get('gap_analysis'):
            return {'critical': 0, 'high': 0, 'medium': 0, 'low': 0}
        
        try:
            gap_file_path = self.base_dir / self.data['gap_analysis']
            with open(gap_file_path, 'r', encoding='utf-8') as f:
                content = f.read()
            
            stats = {'critical': 0, 'high': 0, 'medium': 0, 'low': 0}
            
            # Procurar por linhas com emojis de severidade na tabela de gaps prioritários
            lines = content.split('\n')
            for line in lines:
                if '🔴 Crítica' in line:
                    stats['critical'] += 1
            ...
            print(f"   ⚠️  Erro ao extrair estatísticas de gaps: {e}")
            return {'critical': 0, 'high': 0, 'medium': 0, 'low': 0}
    ...
    def generate_html(self) -> str:
        ...
        gap_analysis_html = ''
        if stats['has_gaps']:
            gap_stats = self.data.get('gap_stats', {'critical': 0, 'high': 0})
            gap_analysis_html = f'''
                        <div class="mt-4 space-y-3">
                            <a href="{self.data['gap_analysis']}" target="_blank" class="block p-4 bg-purple-50 rounded-lg hover:bg-purple-100 transition border-2 border-purple-200">
                                <div class="flex items-center justify-between">
                                    <span class="font-semibold text-purple-900">📄 Ver Análise Completa</span>
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                <div class="p-3 bg-red-50 rounded-lg border border-red-200">
                                    <p class="text-sm text-gray-600">Gaps Críticos</p>
                                    <p class="text-2xl font-bold text-red-600">{gap_stats['critical']}</p>
                                </div>
                                <div class="p-3 bg-orange-50 rounded-lg border border-orange-200">
                                    <p class="text-sm text-gray-600">Gaps Altos</p>
                                    <p class="text-2xl font-bold text-orange-600">{gap_stats['high']}</p>
                                </div>
                            </div>
                        </div>
            '''
        else:
            gap_analysis_html = '<div class="mt-4"><p class="text-gray-500 italic">Análise de gaps não disponível</p></div>'
        
        # TO-BE Model HTML
        to_be_html = ''
        if stats['has_to_be']:
            to_be_html = f'''
                        <a href="{self.data['to_be']}" target="_blank" class="block p-4 bg-green-50 rounded-lg hover:bg-green-100 transition border-2 border-green-200">
                            <div class="flex items-center justify-between">
                                <span class="font-semibold text-green-900">📄 Ver Modelo Completo</span>
                        ...
                        <div class="mt-3 p-3 bg-gray-50 rounded-lg">
                            <p class="text-sm text-gray-600 mb-2">Decisões Arquiteturais</p>
                            <ul class="space-y-1 text-sm text-gray-700">
                                <li>• Stack Tecnológico</li>
                                <li>• Padrões Arquiteturais</li>
                                <li>• APIs e Integração</li>
                                <li>• Observabilidade</li>
                            </ul>
                        </div>
            '''
        else:
            to_be_html = '<p class="text-gray-500 italic">Modelo TO-BE não disponível</p>'
        
        # Concept NAV 360 HTML
        concept_html = ''
        if stats['has_concept']:
            concept_html = f'''
                        <a href="{self.data['concept']}" target="_blank" class="block p-4 bg-indigo-50 rounded-lg hover:bg-indigo-100 transition border-2 border-indigo-200">
                            <div class="flex items-center justify-between">
                                <span class="font-semibold text-indigo-900">📄 Ver Concept</span>
                        ...
                        <div class="mt-3 p-3 bg-gray-50 rounded-lg">
                            <p class="text-sm text-gray-600 mb-2">Funcionalidades Estratégicas</p>
                            <ul class="space-y-1 text-sm text-gray-700">
                                <li>• Hero Personalizado</li>
            ...
            concept_html = '<p class="text-gray-500 italic">Concept NAV 360 não disponível</p>'
